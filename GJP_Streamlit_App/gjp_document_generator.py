...
def _replace_section_paragraph_text(doc: DocumentObject, heading: str, replacement_text: str, stop_headings: Iterable[str]) -> None:
    """Replace paragraphs after a heading until the next heading-like stop."""
...
def add_ksa_table_after(paragraph, ksa_rows: List[KsaRow], responsibilities: List[Responsibility], max_numbers_per_line: int = 10) -> object:
    # Use 13 responsibility-number columns. If a KSA maps to more than 10 responsibilities,
    # the numbers are split into additional lines within the same cells for readability.
    resp_by_no = {r.number: r for r in responsibilities}
    table = _insert_table_after(paragraph, rows=len(ksa_rows) + 1, cols=13)
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    table.style = "Table Grid"
    widths = [0.35, 2.6] + [0.42] * 11

    header_cells = ["No.", "Skills"] + ["Responsibilities"] + [""] * 10
    ...
    for r, ksa in enumerate(ksa_rows, start=1):
        table.cell(r, 0).text = str(ksa.number)
        table.cell(r, 1).text = ksa.text
        for c in (0, 1):
            _set_cell_borders(table.cell(r, c))
            _set_cell_width(table.cell(r, c), widths[c])
            _format_cell_text(table.cell(r, c), font_size=7, align=WD_ALIGN_PARAGRAPH.CENTER if c == 0 else WD_ALIGN_PARAGRAPH.LEFT)

        # Put up to 10 numbers in a visual line. If there are more, continue on a new line in same row.
        nums = ksa.responsibilities
        lines = _chunks(nums, max_numbers_per_line) if nums else []
        flat_slots: List[str] = []
        flat_colors: List[str] = []
        for line_index, line in enumerate(lines):
            if line_index > 0:
                # Separate wrapped groups with an empty visual gap.
                flat_slots.append("\n")
                flat_colors.append("FFFFFF")
            for n in line:
                flat_slots.append(str(n))
                flat_colors.append(_theme_color(resp_by_no.get(n, Responsibility(n, "", "")).theme))

        for c in range(2, 13):
            cell = table.cell(r, c)
            slot_index = c - 2
            if slot_index < len(flat_slots):
                cell.text = flat_slots[slot_index]
                _set_cell_fill(cell, flat_colors[slot_index])
            else:
                cell.text = ""
            _set_cell_borders(cell)
            _set_cell_width(cell, widths[c])
...
def replace_tables_in_document(doc: DocumentObject, level_tables: List[LevelTables]) -> None:
    ...
    skills_heading = _clear_tables_after_heading_until(doc, "Skills", ["**Note", "Note"])
